...
GeoAreaController.getCounties = async (req, res) => {
  ...
  GeoAreaController.deteteNonFavorite = async (req, res) => {
    try {
      const userId = req.body.userId;
      const savedType = req.body.savedType;

      const data = await GeoAreaService.deteteNonFavorite(userId, savedType);
      res.sendStatus(200);
    } catch (err) {
      console.log(err);

        res.status(500).send('Error detete non favorite ');
    }
  };
