...
const TeamInvestigatorSecondLevel : React.FC<ComponentProps> = ({ id ,filterCriteria}) => {
  ...
  if (modeFilter && currentTab === '2') {
    const fetchData = async () => {

      TeamService.getTeamSecLevelByFilter({id :id},filterCriteria)
              .then((response: any) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      TeamService.getTeamSecondLevel({id :id})
              .then((response: any) => {
                // setTeamNeo4jData(response.data);
                if(response.data){
                // setNodes(teamNeo4jData.nodes);
    ...
    if (!modeFilter &&  (currentTab === '' || currentTab === '2')) {
    fetchData();
    }
  }, [id,filterCriteria]);
  ...
  useEffect(() => {
    ...
    console.log(teamNeo4jData.links);
  ...
  return (  
    <div>
      ...
      <ForceGraph2D
        ...
        height={fullScreenActivated?dimensions.height:620} // 500 second level
        // nodeAutoColorBy="agentoffice"
        // maxZoom={10} //10 old value
        // minZoom={3} //5 old value
...
export default TeamInvestigatorSecondLevel;
