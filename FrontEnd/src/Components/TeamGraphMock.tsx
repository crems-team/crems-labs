...
const TeamGraphMock : React.FC<ComponentProps> = ({ teamName, filterCriteria }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      TeamService.getTeamMock({name : teamName})
              .then((response: any) => {
                // setTeamNeo4jData(response.data);
                if(response.data){
                // setNodes(teamNeo4jData.nodes);
  ...
  useEffect(() => {
   if (!teamNeo4jData) return;

    const baseNodes = teamNeo4jData.nodes;
    const baseLinks = processedLinks;
    ...
    const filteredNodes = baseNodes.filter((n: any) => {
      // hub
      if (n.tier === 0) return true;

      // filtrer par tiers
      if (n.tier && hiddenTiers.has(n.tier)) {
        return false;
      }

      // filtrer admin
      if (filterCriteria?.hideAdmin && n.admin === 'O') {
        return false;
      }
    ...
    const allowedIds = new Set(filteredNodes.map((n: any) => n.id));

    const filteredLinks = baseLinks.filter((l: any) =>
      allowedIds.has(l.source) && allowedIds.has(l.target)
    );

  // 5) mise à jour du state affiché
  setNodes(filteredNodes);
  setLinks(filteredLinks);

  // 6) MAJ des positions initiales pour reset drag
  filteredNodes.forEach((node: NodeObject) => {
    if (node.id) {
      initialPositions.current[node.id] = { x: node.x!, y: node.y! };
    }
  });

}, [teamNeo4jData, processedLinks, filterCriteria]);
  ...
  const redirectToAPR = () => {
    ...
    navigate(`/AgentProdReports/${teamName}`);
  };
