...
///
  ...
  const saveSearchHistory = async (savedType :string, teamName: string) => {
    if (keycloak.tokenParsed?.sub) {
      ...
      if (!history.some((item :SearchItemHistory)=> item.teamName === teamName)) {
        ...
        try { 
          ...
          TeamService.getSavedSearches(userId, "team")
                        .then((response: any) => {
...
const fetchSavedSearches =  async() => {
  if (keycloak.tokenParsed?.sub) {
    ...
    const userId = keycloak.tokenParsed.sub;
      ...
      await TeamService.getSavedSearches(userId, "team")
      .then((response: any) => {
...
const handleSearch = async(name : string) => {
  setIsLoadingSearchAgent(true);
    var data = {
      ...
      await TeamService.getAgentByTeamName(data)
      .then((response: any) => {
        setDataAgent(response.data);
        saveSearchHistory("team", name);
...
const buttonDataTable = (rowData : any) => {
  ...
  const redirectToTeamGraph = (teamName : string) => {
    setVisibleRight(false);

    navigate(`/teamGraph/${teamName}`);
  };
...
///
  ...
  try {
    ...
    const resp = await TeamService.getTeamByName({ term: query });
                        ...
                        {isLoadingSearchAgent ? (
                            ...
                            <>
                                ...
                                <Button
                                    ...
                                    onClick={() => navigate(`/teamGraph/${name}`)}  
                                />
                                </div>
                                ...
                                <Column field="agentPhone" header="Agent Phone" />
                                <Column field="agentEmail" header="Agent Email" />
                                </DataTable>
                            </>
                            )
                        )}
              ...
              <Sidebar visible={visibleRight} position="right" onHide={() => setVisibleRight(false)} style={{ width: '50rem' }}>
                <div className="col-12  mx-auto">
                  <SearchHistory
                    ...
                    onSearchClick={(search: any) => redirectToTeamGraph(search.teamName)}
